...
#ifndef SCSS_NO_RAMP_TEXTURE
#define SCSS_NO_RAMP_TEXTURE 0
#endif
...
float sharpStep(float rampPosition, half softness)
{
    float shadeWidth = max(fwidth(rampPosition), 0.0002 * (1+softness*10));
...
{
    #if defined(SCSS_NO_RAMP_TEXTURE)
        return sharpStep(rampPosition, softness);
    #else
