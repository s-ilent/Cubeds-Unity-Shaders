v2g vert(appdata_full v) {
	...
	o.extraData.x *= min(distance(o.posWorld,_WorldSpaceCameraPos), 1); 
...
float4 frag_simple(VertexOutput i, uint facing : SV_IsFrontFace) : SV_Target
{
    ...
    half3 normalTangent = NormalInTangentSpace(texcoords, 1);
    ...
    c.normal = normalize(calcedNormal);
	...
	c.alpha = Alpha(texcoords.xy);
		...
		c.alpha = ((c.alpha - _Cutoff) / max(fwidth(c.alpha), 0.0001));
		clip (c.alpha);
	#endif
	...
	// Lighting handling
	float3 finalColor = SCSS_ApplyLightingSimple(c, d, i, viewDir, l, texcoords);
